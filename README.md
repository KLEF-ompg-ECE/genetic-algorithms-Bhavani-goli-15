...
**Student Name  :** G. Krishna Bhavani  
...
```
The fitness() function returns the total value of the packed items if the total weight is less than or equal to the MAX_WEIGHT (15 kg), and returns 0 if the total weight exceeds the limit. This design ensures that invalid solutions (overweight backpacks) are never selected as parents in the genetic algorithm, as they provide no fitness advantage.
```

**Q2. What does `tournament_select()` do? Why are higher-fitness individuals more likely to be chosen?**

```
The tournament_select() function randomly selects k individuals from the population, compares their fitness values, and returns the one with the highest fitness. Higher-fitness individuals are more likely to be chosen because when multiple random candidates are compared, the fittest individual among them consistently wins, giving better solutions a higher probability of being selected for reproduction.
```
...
```
This line implements elitism by copying the best chromosome found so far and placing it directly into the next generation without modification. It's important to always keep the best solution because it guarantees that the optimal solution found up to that point is never lost due to crossover or mutation, which could potentially destroy the best-performing individual.
```
